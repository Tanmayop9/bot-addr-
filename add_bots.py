...
class _SSLAdapter(HTTPAdapter):
    ...
    Some servers close the TLS connection without sending a ``close_notify``
    alert.  Python 3.12 made the OpenSSL default stricter about this, causing
    ``ssl.SSLEOFError: UNEXPECTED_EOF_WHILE_READING``.  Setting
    ``OP_IGNORE_UNEXPECTED_EOF`` (added in Python 3.11) restores the lenient
    behaviour so those servers keep working.
    """
